...
cvfit = cv.glmnet(x = x, y = y, family = "gaussian", nlambda = 100, alpha = 0.8, standardize=TRUE,nfolds=10)
plot(cvfit)
fit = glmnet(x = x, y = y, family = "gaussian",lambda=cvfit$lambda.1se,alpha=0.8, standardize=TRUE)
(fit$beta)
```
...
library(gridExtra)
library(grid)
err = c(cv.err1[2],cv.err3[2],cv.err4[2],cv.err5[2])
