...
```{r}
err = (495/data$DENSITY-450-data$BODYFAT) #siri's formula
which(err==min(err))
```
...
###2.Visualization
Most x's have a positive trend with y. And the multilinearity is heavy in this data set. Age here has a negative trend with height, may because of age range in this data set.
```{r}
library('corrplot')
...
grid.draw(gt)
v = data.frame(VIF=round(v,2))

grid.table(r,main="VIF")
par(mfrow = c(1,1))
plot(model.raw, which=4)
abline( h = 4/(248-15),lty=2 )
```

remove abnormal points and high influential points from the data

#(1)remove high influencial point 39,86,221
...
rule: cook's distance>4/(n-p), one each time(stop at point 86)

rule: no extreme values on other independent variables: 31,41,79
```{r}
data.2 = data[-c(182, 216, 96, 42, 39, 221, 86, 192, 41, 250, 163),-2]
model.raw2 <- lm(BODYFAT ~ ., data = data.2)
plot(model.raw2, which=4)
abline( h = 4/(241-15),lty=2 )
model.raw2 <- lm(BODYFAT ~ ., data = data.2)
hat.plot(model.raw2)
...
Now, we use cleaned data to refit the multivariate linear regression model and we find that the adjusted R-square is 0.71, which is high enough. Furthermore, we can inspect the diagnostic plots. There is not any obvious outlier. From the plot we can guess that except for the normality assumption, no assumption of linear regression model is violated.
```{r}
...
add heigh^2, 1/height or log(height) to the model: no increase in adj.R, but makes the model complex.
```{r}
data.3=data.2
...
#(2)use stepwise regression(AIC): AGE + WEIGHT + HEIGHT + ADIPOSITY + ABDOMEN + THIGH + BICEPS + WRIST
```{r}
model.AIC <- step(model.full, k=2,direction="both")
# use stepwise regression(BIC):WRIST+WEIGHT+ABDOMEN
model.BIC <- step(model.full, k=log(242),direction="both")
```
